...
export async function GET() {
  ...
  const items = await db.select({ category: categories, activityCount: sql<number>`count(${posts.id})` })
    .from(categories)
    .leftJoin(posts, eq(posts.categoryId, categories.id))
    .where(eq(categories.scope, "BLOG"))
    .groupBy(categories.id)
    .orderBy(asc(categories.sortOrder), asc(categories.name));
  return Response.json({ items: items.map(({ category, activityCount }) => ({ ...category, activityCount: Number(activityCount) })) });
}
