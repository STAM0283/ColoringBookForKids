import { BookOpen, CheckCircle2, FileText, Film, HardDrive, ImageIcon, ServerCog } from "lucide-react";
...
export function BackupSettings({ sizes, counts, storageCapacity, databaseIntegrity, hostingProvider, disk }: { sizes:Sizes; counts:Counts; storageCapacity:number; databaseIntegrity:boolean; hostingProvider:string; disk:Disk }) {
  ...
  return <div className="space-y-6">
    <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-700">Vue d’ensemble</p><h2 className="mt-2 text-2xl font-black">Contenu du site</h2><p className="mt-1 text-sm text-slate-500">Comptages PostgreSQL et tailles réelles des fichiers rapportés au stockage configuré.</p></div><p className="rounded-full bg-slate-100 px-4 py-2 text-xs font-bold text-slate-500">Médias : {formatBytes(totalMedia)}</p></div>
