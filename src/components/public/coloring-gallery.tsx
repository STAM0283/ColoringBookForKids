...
export function ColoringGallery({ items, locale = "fr" }: {
    ...
    return <><section aria-label={en ? "Colouring filters" : "Filtres des coloriages"} className="relative z-40 mb-7 overflow-visible rounded-[1.75rem] border bg-card/90 p-4 shadow-[0_20px_55px_-42px_rgba(15,23,42,.7)] backdrop-blur dark:border-white/10 sm:p-5">
...
function FilterSelect({ icon, label, value, change, options }: {
    ...
    setOpen(false); } document.addEventListener("mousedown", outside); document.addEventListener("keydown", escape); return () => { document.removeEventListener("mousedown", outside); document.removeEventListener("keydown", escape); }; }, []); return <div ref={root} className={`relative ${open?"z-50":"z-0"}`}><button type="button" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen(current => !current)} className={`flex min-h-14 w-full items-center gap-3 rounded-2xl border bg-background/95 px-4 text-left transition ${open ? "border-primary ring-4 ring-primary/10" : "hover:border-primary/40 dark:border-white/10"}`}><span className="text-primary">{icon}</span><span className="min-w-0 flex-1"><span className="block text-[9px] font-black uppercase tracking-[.14em] text-foreground/40">{label}</span><span className="block truncate text-sm font-black text-foreground">{selected.label}</span></span><ChevronDown size={17} className={`shrink-0 text-foreground/40 transition ${open ? "rotate-180 text-primary" : ""}`}/></button>{open && <div role="listbox" aria-label={label} className="absolute left-0 right-0 top-[calc(100%+.5rem)] z-[100] max-h-[min(22rem,55vh)] overflow-y-auto overscroll-contain rounded-2xl border bg-card p-2 shadow-[0_24px_60px_-18px_rgba(15,23,42,.45)] dark:border-white/10"><p className="sticky top-0 z-10 rounded-t-xl bg-card px-3 pb-2 pt-1 text-[9px] font-black uppercase tracking-[.14em] text-foreground/40">{label}</p>{options.map(option => <button key={option.value || "all"} type="button" role="option" aria-selected={option.value === value} onClick={() => { change(option.value); setOpen(false); }} className={`flex min-h-11 w-full items-center justify-between gap-3 rounded-xl px-3 text-left text-sm font-bold transition ${option.value === value ? "bg-primary text-white shadow-sm" : "text-foreground hover:bg-primary/10 hover:text-primary"}`}><span className="truncate">{option.label}</span>{option.value === value && <Check className="shrink-0" size={17}/>}</button>)}</div>}</div>; }
type Action = {
    zone: string;
    before: string;
    after: string;
...
function StudioShell(props: StudioProps) {
    ...
    <main className="coloring-studio-main mx-auto grid min-h-0 w-full max-w-7xl flex-1 p-1.5 pb-[7.75rem] sm:p-2 sm:pb-[8rem] lg:grid-cols-[250px_1fr] lg:gap-5 lg:p-4">
