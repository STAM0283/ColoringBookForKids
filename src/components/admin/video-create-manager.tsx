...
function CreateVideoDialog({ close }: { close: () => void }) {
  ...
  return <div className="fixed inset-0 z-[120] grid place-items-center bg-slate-950/75 p-3 backdrop-blur-md sm:p-5" onMouseDown={dismiss}>
    <form ref={formRef} onSubmit={submit} onMouseDown={event => event.stopPropagation()} className="video-create-dialog flex max-h-[calc(100dvh-1.5rem)] w-full max-w-6xl flex-col overflow-hidden rounded-[1.5rem] border border-transparent bg-white shadow-[0_35px_120px_-30px_rgba(0,0,0,.8)] dark:border-white/10 dark:bg-slate-900 dark:[color-scheme:dark] sm:max-h-[calc(100dvh-2.5rem)] sm:rounded-[2rem]">
      ...
      <div className="grid min-h-0 flex-1 gap-5 overflow-y-auto overscroll-contain px-5 py-5 sm:px-7 lg:grid-cols-[1.08fr_.92fr]">
