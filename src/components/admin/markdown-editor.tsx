...
export function MarkdownEditor({ value, onChange, name, label="Contenu Markdown", help="Rédigez, puis vérifiez le rendu avec l’aperçu.", maximumLength=100000, minimumLength=10, required=true, compact=false }: MarkdownEditorProps) {
  ...
  return <div className="min-w-0">
    <div className="mb-2 flex flex-col flex-wrap justify-between gap-3 sm:flex-row sm:items-end">
      <div>
        <p className="flex items-center gap-1 text-sm font-bold text-slate-800 dark:text-slate-100">{label} {required&&<><span aria-hidden="true" className="text-rose-600">*</span><span className="sr-only">(obligatoire)</span></>}</p>
      ...
      <div className="markdown-editor-tabs flex w-fit rounded-xl bg-slate-100 p-1 dark:bg-white/[.06]">
        <button type="button" aria-pressed={mode==="write"} onClick={()=>setMode("write")} className={`${tab} ${mode==="write"?"is-active bg-white text-emerald-700 shadow-sm dark:bg-emerald-400/15 dark:text-emerald-300":"text-slate-500 dark:text-slate-300"}`}><FileCode2 size={16}/> Markdown</button>
        <button type="button" aria-pressed={mode==="preview"} onClick={()=>setMode("preview")} className={`${tab} ${mode==="preview"?"is-active bg-white text-emerald-700 shadow-sm dark:bg-emerald-400/15 dark:text-emerald-300":"text-slate-500 dark:text-slate-300"}`}><Eye size={16}/> Aperçu</button>
      </div>
    </div>
    {mode==="write"?<textarea aria-label={label} value={value} minLength={minimumLength} maxLength={maximumLength} onChange={event=>onChange(event.target.value)} className={`${height} w-full rounded-xl border px-4 py-4 font-mono text-sm font-normal leading-6 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 dark:border-white/10 dark:bg-slate-950/45 dark:text-slate-100 dark:focus:ring-emerald-500/15`} placeholder="# Mon titre&#10;&#10;Un paragraphe avec du **gras**…" required={required}/>:<div className={`${height} rounded-xl border bg-[#fdfbf7] p-5 transition-colors dark:border-white/10 dark:bg-slate-950/45`}><p className="mb-4 border-b pb-3 text-xs font-black uppercase tracking-[.16em] text-emerald-700 dark:border-white/10 dark:text-emerald-300">Aperçu du contenu</p>{value.trim()?<div className="rich-article break-words text-base font-normal leading-7 text-slate-700 dark:text-slate-200" dangerouslySetInnerHTML={{__html:articleContentToHtml(value)}}/>:<div className={`grid ${emptyHeight} place-items-center text-center text-sm text-slate-400 dark:text-slate-500`}>Le rendu apparaîtra ici après avoir ajouté du Markdown.</div>}</div>}
