...
export function BackupSettings({ sizes, counts, storageCapacity, databaseIntegrity, disk }: { sizes:Sizes; counts:Counts; storageCapacity:number; databaseIntegrity:boolean; disk:Disk }) {
  ...
  return <div className="space-y-6">
    <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-700">Vue d’ensemble</p><h2 className="mt-2 text-2xl font-black">Contenu du site</h2><p className="mt-1 text-sm text-slate-500">Comptages SQLite et tailles réelles des fichiers rapportés aux 40 Go du VPS OVH.</p></div><p className="rounded-full bg-slate-100 px-4 py-2 text-xs font-bold text-slate-500">Médias : {formatBytes(totalMedia)}</p></div>
      <div className="mt-8 grid items-stretch gap-5 lg:grid-cols-[minmax(320px,.65fr)_minmax(0,1.35fr)]">
        <DatabaseHealth sitePercent={siteUsage} siteUsed={totalUsed} planCapacity={storageCapacity} databaseSize={sizes.database} integrity={databaseIntegrity} disk={disk}/>
...
function DatabaseHealth({sitePercent,siteUsed,planCapacity,databaseSize,integrity,disk}:{sitePercent:number;siteUsed:number;planCapacity:number;databaseSize:number;integrity:boolean;disk:Disk}){const diskPercent=disk.total?Math.min(100,disk.used/disk.total*100):0,color=diskPercent>=85?"#dc2626":diskPercent>=70?"#d97706":"#16a34a";return <div className="flex h-full min-h-72 flex-col items-center justify-center rounded-[1.5rem] border border-emerald-100 bg-gradient-to-br from-emerald-50/80 via-white to-white p-6 text-center shadow-sm"><p className="text-xs font-black uppercase tracking-[.18em] text-emerald-700">{disk.isProduction?"Disque du serveur OVH":"Disque de développement local"}</p><div className="relative mt-5 size-44"><Ring percent={diskPercent} color={color} track="#dcfce7"/><span className="absolute inset-0 grid place-items-center"><span><HardDrive size={23} className="mx-auto mb-2 text-slate-400"/><strong className="block text-3xl font-black text-slate-900">{formatPercent(diskPercent)}</strong><small className="font-bold text-slate-400">du disque utilisé</small></span></span></div><p className="mt-4 text-xs font-semibold text-slate-500">{formatBytes(disk.used)} utilisés sur {formatBytes(disk.total)} · {formatBytes(disk.available)} libres</p><div className="mt-4 w-full rounded-xl border border-slate-100 bg-white/80 p-3 text-left text-xs"><div className="flex justify-between gap-3"><span className="text-slate-500">Contenus du site</span><strong>{formatBytes(siteUsed)}</strong></div><div className="mt-1.5 flex justify-between gap-3"><span className="text-slate-500">Part du forfait OVH ({formatBytes(planCapacity)})</span><strong>{formatPercent(sitePercent)}</strong></div></div><div className={`mt-3 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px] font-black ${integrity?"bg-emerald-100 text-emerald-700":"bg-red-100 text-red-700"}`}><CheckCircle2 size={14}/> SQLite {integrity?"intègre":"à vérifier"} · {formatBytes(databaseSize)}</div></div>}
