...
import { contentLanguage, localizedPath, type Locale } from "@/lib/i18n";

export async function HomePage({locale="fr"}:{locale?:Locale}) {
  const language=contentLanguage(locale),en=locale==="en";
  const [books, activities, posts, vlogs, clubUnlocked] = await Promise.all([
    contentRepository.books({ pageSize: 3,language }),
    contentRepository.activities({ pageSize: 4,language }),
    contentRepository.posts({ pageSize: 3,language }),
    contentRepository.vlogs({ pageSize: 3,language }),
    hasClubAccess(),
  ]);
    ...
    <>
        ...
        <div className="container grid items-center gap-12 lg:grid-cols-[1.04fr_.96fr] lg:gap-16">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-primary/15 bg-card/80 px-4 py-2 text-sm font-black text-primary shadow-sm backdrop-blur dark:bg-white/5">
              <Sparkles size={16} aria-hidden="true" /> {en?"Creativity starts here":"La créativité commence ici"}
            </span>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={localizedPath("books",locale)} className="focus-ring group inline-flex min-h-14 items-center gap-2 rounded-full bg-primary px-7 font-black text-white shadow-lg shadow-primary/20 transition duration-300 hover:-translate-y-1 hover:shadow-xl">
                {en?"Discover the books":"Découvrir les livres"} <ArrowRight size={18} className="transition group-hover:translate-x-1"/>
              </Link>
              <Link href={localizedPath("activities",locale)} className="focus-ring group inline-flex min-h-14 items-center gap-2 rounded-full border bg-card px-7 font-black shadow-sm transition duration-300 hover:-translate-y-1 hover:border-amber-400 hover:text-amber-700 hover:shadow-lg dark:hover:text-amber-300">
                <Download size={18}/> {en?"Activities":"Activités"}
              </Link>
            </div>
            <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm font-bold text-foreground/55">
              <span className="inline-flex items-center gap-2"><span className="grid size-7 place-items-center rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-300">✓</span> {en?"Ready to print":"À imprimer"}</span>
              <span className="inline-flex items-center gap-2"><span className="grid size-7 place-items-center rounded-full bg-blue-100 text-blue-700 dark:bg-blue-400/15 dark:text-blue-300">✓</span> {en?"Made for children":"Adapté aux enfants"}</span>
              <span className="inline-flex items-center gap-2"><span className="grid size-7 place-items-center rounded-full bg-rose-100 text-rose-700 dark:bg-rose-400/15 dark:text-rose-300">✓</span> {en?"Created with care":"Imaginé avec cœur"}</span>
            </div>
          </div>
            ...
            <div className="absolute inset-11 -rotate-3 overflow-hidden rounded-[3rem] border-[10px] border-white bg-gradient-to-br from-[#e4f3e8] to-[#dce9df] shadow-2xl dark:border-white/10 dark:from-emerald-950 dark:to-slate-900">
              ...
              <div className="relative grid h-full place-items-center text-center">
                <div>
                  ...
                  <p className="mt-5 font-display text-2xl font-black">{en?"Imagination":"L’imagination"}<br/><span className="text-primary">{en?"in full color":"en couleurs"}</span></p>
      ...
      <section className="container relative z-10 -mt-2 sm:-mt-5">
        <div className="grid gap-3 rounded-[2rem] border bg-card/90 p-3 shadow-[0_24px_70px_-42px_rgba(15,23,42,.5)] backdrop-blur md:grid-cols-3 dark:border-white/10">
          <HomeChoice href={localizedPath("books",locale)} icon={<BookOpen/>} eyebrow={en?"Explore":"Explorer"} title={en?"Choose a story":"Choisir une histoire"} color="bg-amber-100 text-amber-700 dark:bg-amber-400/15 dark:text-amber-300"/>
          <HomeChoice href={localizedPath("activities",locale)} icon={<Palette/>} eyebrow={en?"Create":"Créer"} title={en?"Print an activity":"Imprimer une activité"} color="bg-rose-100 text-rose-700 dark:bg-rose-400/15 dark:text-rose-300"/>
          <HomeChoice href={localizedPath("videos",locale)} icon={<PlayCircle/>} eyebrow={en?"Watch":"Regarder"} title={en?"Enter the studio":"Entrer dans l’atelier"} color="bg-blue-100 text-blue-700 dark:bg-blue-400/15 dark:text-blue-300"/>
        </div>
      </section>

      <HomeSection title={en?"Choose their next adventure":"Choisissez sa prochaine aventure"} eyebrow={en?"The collection":"Le catalogue"} href={localizedPath("books",locale)} viewAll={en?"View all":"Tout voir"} compact>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {books.items.length ? books.items.map(item => <HomeBookCard key={item.book.id} book={item.book} coverPath={item.cover?.path} locale={locale}/>) : <EmptyState locale={locale} />}
        </div>
      </HomeSection>

      <HomeSection title={en?"Print, color and share":"À imprimer, à colorier, à partager"} eyebrow={en?"100% free":"100 % gratuit"} href={localizedPath("activities",locale)} viewAll={en?"View all":"Tout voir"} tint>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {activities.items.length ? activities.items.map((item, index) => <ActivityCard key={item.id} item={item} index={index} clubUnlocked={clubUnlocked} locale={locale}/>) : <EmptyState locale={locale} />}
        </div>
      </HomeSection>

      <HomeSection title={en?"Inside the studio":"Dans l’atelier"} eyebrow={en?"Latest videos":"Dernières vidéos"} href={localizedPath("videos",locale)} viewAll={en?"View all":"Tout voir"}>
        <div className="grid gap-6 md:grid-cols-3">
          {vlogs.items.length ? vlogs.items.map(({vlog, thumbnail}) => <VlogCard key={vlog.id} item={vlog} thumbnailPath={thumbnail?.path} locale={locale}/>) : <EmptyState locale={locale} />}
        </div>
      </HomeSection>

      <HomeSection title={en?"Create and grow together":"Pour créer et grandir ensemble"} eyebrow={en?"The journal":"Le journal"} href={localizedPath("blog",locale)} viewAll={en?"View all":"Tout voir"}>
        <div className="grid gap-6 md:grid-cols-3">
          {posts.items.length ? posts.items.map(({post, cover}) => <PostCard key={post.id} item={post} coverPath={cover?.path} locale={locale}/>) : <EmptyState locale={locale} />}
        </div>
      </HomeSection>
...
function HomeBookCard({book,coverPath,locale}:{book:{slug:string;title:string;shortDescription:string;ageMin:number|null;ageMax:number|null;pageCount:number|null;pricingType:"FREE"|"PAID"};coverPath?:string;locale:Locale}) {
  const en=locale==="en",age=book.ageMin!==null&&book.ageMax!==null?`${book.ageMin}–${book.ageMax} ${en?"years":"ans"}`:null;
  return <Link href={`${localizedPath("books",locale)}/${book.slug}`} className="group grid min-h-48 overflow-hidden rounded-[1.5rem] border bg-card shadow-[0_16px_45px_-34px_rgba(15,23,42,.55)] transition hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl sm:grid-cols-[150px_1fr]">
    <div className="relative min-h-48 overflow-hidden bg-primary/10">{coverPath?<Image src={`/media/${coverPath}`} alt={`${en?"Cover of":"Couverture de"} ${book.title}`} fill quality={90} sizes="150px" className="object-contain p-3 transition duration-500 group-hover:scale-105"/>:<span className="grid size-full place-items-center text-primary"><BookOpen size={38}/></span>}</div>
    <div className="flex min-w-0 flex-col justify-center p-5"><div className="flex flex-wrap gap-2">{age&&<span className="rounded-full bg-secondary px-2.5 py-1 text-[10px] font-black uppercase">{age}</span>}{book.pageCount&&<span className="rounded-full bg-secondary px-2.5 py-1 text-[10px] font-black uppercase">{book.pageCount} pages</span>}<span className="rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-black uppercase text-blue-700">{book.pricingType==="FREE"?(en?"Free":"Gratuit"):(en?"Paid":"Payant")}</span></div><h3 className="mt-3 text-balance font-display text-xl font-black leading-tight">{book.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-relaxed text-foreground/55">{book.shortDescription}</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-black text-primary">{en?"View book":"Voir le livre"} <ArrowRight size={16} className="transition group-hover:translate-x-1"/></span></div>
  </Link>;
}

function HomeSection({ title, eyebrow, href, viewAll, children, tint = false, compact = false }: { title: string; eyebrow: string; href: string; viewAll:string; children: React.ReactNode; tint?: boolean; compact?: boolean }) {
  return (
