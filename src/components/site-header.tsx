...
export function SiteHeader() {
  const[open,setOpen]=useState(false),[galleryOpen,setGalleryOpen]=useState(false),[mobileGalleryOpen,setMobileGalleryOpen]=useState(false);
  ...
  return <header className="sticky top-0 z-40 border-b bg-background/90 shadow-[0_8px_30px_-24px_rgba(15,23,42,.55)] backdrop-blur-xl">
    <div className="container flex h-16 items-center justify-between gap-3 sm:h-20">
      ...
      <button type="button" aria-label={open?(locale==="en"?"Close menu":"Fermer le menu"):(locale==="en"?"Open menu":"Ouvrir le menu")} aria-expanded={open} aria-controls="mobile-navigation" className="focus-ring grid size-11 shrink-0 place-items-center rounded-2xl border bg-card text-foreground shadow-sm transition hover:border-primary/35 hover:bg-primary/10 hover:text-primary lg:hidden" onClick={()=>setOpen(value=>!value)}>{open?<X size={22}/>:<Menu size={23}/>}</button>
    </div>
    {open&&<div id="mobile-navigation" className="absolute inset-x-0 top-full z-50 h-[calc(100dvh-4rem)] overflow-y-auto border-t bg-background shadow-2xl dark:border-white/10 sm:h-[calc(100dvh-5rem)] lg:hidden">
      <nav aria-label={locale==="en"?"Mobile navigation":"Navigation mobile"} className="container grid gap-2 py-5">
        {navigation.map(item=>item==="gallery"?<div key={item} className="overflow-hidden rounded-2xl border bg-card shadow-sm dark:border-white/10">
          <button type="button" aria-expanded={mobileGalleryOpen} onClick={()=>setMobileGalleryOpen(value=>!value)} className={cn("flex min-h-14 w-full items-center justify-between px-3 font-bold text-foreground/80",galleryActive&&"text-primary")}><span className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-primary/10 text-primary"><ImagesIcon size={19}/></span>{galleryLabel}</span><ChevronDown size={18} className={`mr-2 transition ${mobileGalleryOpen?"rotate-180":""}`}/></button>
