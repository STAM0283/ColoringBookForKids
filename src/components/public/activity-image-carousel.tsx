...
export function ActivityImageCarousel({ activityId, coverPath, images, title, locale = "fr" }: { activityId:string;coverPath?:string|null;images: ActivityImage[]; title: string; locale?: "fr" | "en" }) {
  ...
  const model = (fullscreen=false) => showModel&&coverPath&&current.path!==coverPath?<div className={`absolute right-2 top-2 z-20 overflow-hidden rounded-xl border-4 border-white bg-white shadow-xl ${fullscreen?"h-[24%] w-[24%] max-w-64":"h-[26%] w-[26%]"}`}><Image src={`/media/${coverPath}`} alt={en?"Color model":"Modèle en couleur"} fill sizes={fullscreen?"260px":"140px"} className="object-contain"/></div>:null;
  ...
  return <>
    <div className="relative size-full overflow-hidden">
      ...
      {model()}
      <button type="button" onClick={() => setExpanded(true)} aria-label={en ? "Open large preview" : "Ouvrir l’aperçu en grand"} className="absolute left-3 top-3 z-20 grid size-10 place-items-center rounded-full border border-white/50 bg-white/90 text-slate-800 shadow-lg backdrop-blur transition hover:scale-105 hover:bg-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/25"><Expand size={17}/></button>
