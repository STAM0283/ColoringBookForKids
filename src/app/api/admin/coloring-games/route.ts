...
const fields=z.object({language:z.enum(["FR","EN"]),title:z.string().trim().min(3).max(100),description:z.string().trim().min(10).max(400),difficulty:z.enum(["EASY","MEDIUM","HARD"]),ageMin:z.number().int().min(2).max(18).nullable(),ageMax:z.number().int().min(2).max(18).nullable(),published:z.boolean(),sortOrder:z.number().int().min(0).max(9999),characterIds:z.array(z.string().uuid()).max(12)}).refine(value=>value.ageMin===null||value.ageMax===null||value.ageMax>=value.ageMin,{message:"La tranche d’âge est invalide."});
async function isAdmin(){return(await auth())?.user.role==="ADMIN"}
export async function GET(){if(!await isAdmin())return Response.json({message:"Non autorisé."},{status:401});const rows=await db.select({game:coloringGames,imagePath:media.path}).from(coloringGames).leftJoin(media,eq(coloringGames.sourceMediaId,media.id)).orderBy(asc(coloringGames.language),asc(coloringGames.sortOrder),asc(coloringGames.title)),items=rows.map(row=>row.game);const links=items.length?await db.select().from(coloringGameCharacters).where(inArray(coloringGameCharacters.coloringGameId,items.map(item=>item.id))):[];return Response.json({items:rows.map(row=>({...row.game,imagePath:row.imagePath,characterIds:links.filter(link=>link.coloringGameId===row.game.id).map(link=>link.characterId)}))})}
export async function POST(request:Request){
 if(!await isAdmin())return Response.json({message:"Non autorisé."},{status:401});
 try{
  const form=await request.formData(),file=form.get("svg"),parsed=fields.safeParse({language:form.get("language"),title:form.get("title"),description:form.get("description"),difficulty:form.get("difficulty"),ageMin:form.get("ageMin")?Number(form.get("ageMin")):null,ageMax:form.get("ageMax")?Number(form.get("ageMax")):null,published:form.get("published")==="true",sortOrder:Number(form.get("sortOrder")||0),characterIds:String(form.get("characterIds")||"").split(",").filter(Boolean)});
