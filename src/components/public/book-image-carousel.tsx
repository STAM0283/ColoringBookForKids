...
export function BookImageCarousel({ images, title, fallback }: { images: CarouselImage[]; title: string; fallback: React.ReactNode }) {
    ...
    <div role="dialog" aria-modal="true" aria-label={`${en ? "Preview of" : "Aperçu de"} ${title}`} className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/95 p-4 backdrop-blur-md" onClick={() => setExpanded(false)}>
      <div className="relative h-full max-h-[92vh] w-full max-w-6xl" onClick={event => event.stopPropagation()}>
        ...
        {images.length > 1 && <>
          ...
          <button type="button" aria-label={en ? "Next image" : "Image suivante"} onClick={next} className="absolute right-2 top-1/2 z-10 grid size-12 -translate-y-1/2 place-items-center rounded-full bg-white text-slate-900 shadow-xl transition hover:scale-105"><ChevronRight /></button>
  ...
  return <>
    <div className="relative size-full overflow-hidden">
      ...
      {images.length > 1 && <>
        <button type="button" aria-label={en ? "Previous image" : "Image précédente"} onClick={previous} className="absolute left-3 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-white/95 text-slate-800 shadow-md transition hover:scale-105"><ChevronLeft /></button>
        <button type="button" aria-label={en ? "Next image" : "Image suivante"} onClick={next} className="absolute right-3 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-white/95 text-slate-800 shadow-md transition hover:scale-105"><ChevronRight /></button>
        <div className="absolute bottom-2 left-1/2 flex -translate-x-1/2 rounded-full bg-slate-950/60 px-1.5 py-1 backdrop-blur">{images.map((_, dot) => <button type="button" key={dot} aria-label={`${en ? "Show image" : "Afficher l’image"} ${dot + 1}`} onClick={() => setIndex(dot)} className="grid size-8 place-items-center rounded-full"><span className={`h-2 rounded-full transition-all ${dot === index ? "w-5 bg-white" : "w-2 bg-white/50"}`}/></button>)}</div>
