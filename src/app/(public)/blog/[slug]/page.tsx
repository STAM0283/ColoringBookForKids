...
import { readingTimeLabel, readingTimeMinutes } from "@/lib/reading-time";
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const result = await getPost(slug);
  if (!result) notFound();
  const structuredData = { "@context": "https://schema.org", "@type": "BlogPosting", headline: result.post.title, description: result.post.excerpt, url: siteUrl(`/blog/${slug}`), image: mediaUrl(result.cover?.path), timeRequired: `PT${readingTimeMinutes(result.post.content)}M`, datePublished: result.post.publishedAt?.toISOString(), dateModified: result.post.updatedAt.toISOString(), author: { "@type": "Organization", name: result.post.authorName || SITE_NAME }, publisher: { "@type": "Organization", name: SITE_NAME } };
  ...
  return <article className="container max-w-4xl py-16 md:py-24"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd(structuredData) }}/>
    ...
    <div className="mt-5 flex flex-wrap items-center gap-3 text-sm font-semibold text-foreground/60"><span>Par {result.post.authorName}</span><span aria-hidden="true">·</span><span className="inline-flex items-center gap-1.5"><Clock3 size={16}/>{readingTimeLabel(result.post.content)}</span></div>
