...
export default async function EnglishBlogPost({params}:{params:Promise<{slug:string}>}){const{slug}=await params,result=await getPost(slug);if(!result)notFound();return <article className="container max-w-4xl py-16"><Link href="/en/blog" className="inline-flex items-center gap-2 rounded-full border bg-card px-4 py-2.5 font-bold"><ArrowLeft size={18}/>Back to the journal</Link><p className="mt-10 text-sm font-black uppercase tracking-[.2em] text-primary">The journal</p><h1 className="mt-4 font-display text-4xl font-black md:text-6xl">{result.post.title}</h1><p className="mt-5 text-lg leading-8 text-foreground/75">{result.post.excerpt}</p><div className="mt-5 flex flex-wrap items-center gap-3 text-sm font-semibold text-foreground/60"><span>By {result.post.authorName}</span><span aria-hidden="true">·</span><span className="inline-flex items-center gap-1.5"><Clock3 size={16}/>{readingTimeLabel(result.post.content,"en")}</span></div>{result.cover&&<div className="relative mt-10 aspect-video overflow-hidden rounded-[2rem]"><Image fill priority src={`/media/${result.cover.path}`} alt={result.cover.alt||result.post.title} className="object-contain"/></div>}<div className="rich-article mt-10 text-lg leading-8" dangerouslySetInnerHTML={{__html:articleContentToHtml(result.post.content)}}/></article>}
