...
export const contentRepository = {
  async books(options: ListingOptions = {}) {
    ...
    const where = and(eq(books.published, true), eq(books.language, options.language ?? "FR"), options.search ? or(like(books.title, `%${options.search}%`), like(books.shortDescription, `%${options.search}%`)) : undefined, options.category ? eq(categories.slug, options.category) : undefined, options.pricing ? eq(books.pricingType, options.pricing) : undefined);
  ...
  async posts(options: ListingOptions = {}) {
    const pagination = bounds(options), where = and(eq(posts.published, true),eq(posts.language,options.language??"FR"), options.search ? or(like(posts.title, `%${options.search}%`),like(posts.excerpt, `%${options.search}%`)): undefined,options.category?eq(categories.slug,options.category):undefined),dateOrder=options.sort==="oldest"?asc(posts.publishedAt):desc(posts.publishedAt);
  ...
  async vlogs(options: ListingOptions = {}) {
    const pagination = bounds(options), where = and(eq(vlogs.published, true),eq(vlogs.language,options.language??"FR"), options.search ? or(like(vlogs.title, `%${options.search}%`),like(vlogs.description, `%${options.search}%`)): undefined), thumbnail = alias(media, "home_vlog_thumbnail"),dateOrder=options.sort==="oldest"?asc(vlogs.publishedAt):desc(vlogs.publishedAt);
