...
export type ColoringGame = { id:string; title:string; description:string; engine:"SVG"|"RASTER"; svgContent:string; imagePath:string|null; difficulty:"EASY"|"MEDIUM"|"HARD"; ageMin:number|null; ageMax:number|null; zoneCount:number; category:{name:string;slug:string;color:string}|null; characters:Array<{name:string;slug:string;color:string}> };
...
function SvgStudio({ game, en, close }: { game:ColoringGame; en:boolean; close:()=>void }) {
  ...
  function paint(event: React.PointerEvent) { const zone = (event.target as Element).closest<SVGElement>("[data-color-zone]"); if (!zone) return; const before = zone.getAttribute("fill") || "#FFFFFF", after = eraser ? (zone.dataset.originalFill || "#FFFFFF") : color; if (before === after) return; zone.setAttribute("fill", after); updateHistory([...undoHistory.current, { zone:zone.dataset.colorZone || "", before, after }], []); update(); }
  ...
  function undoOne() { const action = undoHistory.current.at(-1); if (!action) return; apply(action, action.before); updateHistory(undoHistory.current.slice(0, -1), [...redoHistory.current, action]); }
  function redoOne() { const action = redoHistory.current.at(-1); if (!action) return; apply(action, action.after); updateHistory([...undoHistory.current, action], redoHistory.current.slice(0, -1)); }
  ...
  async function getCanvas() { const svg = root.current?.querySelector("svg"); if (!svg) return null; const source = new XMLSerializer().serializeToString(svg), blob = new Blob([source], { type:"image/svg+xml" }), url = URL.createObjectURL(blob), image = new window.Image(); return new Promise<HTMLCanvasElement | null>(resolve => { image.onload = () => { const box = svg.getAttribute("viewBox")?.split(/\s+/).map(Number), width = Math.max(800, box?.[2] || 1200), height = Math.max(800, box?.[3] || 1200), canvas = document.createElement("canvas"); canvas.width = width; canvas.height = height; const context = canvas.getContext("2d"); if (!context) return resolve(null); context.fillStyle = "#fff"; context.fillRect(0, 0, width, height); context.drawImage(image, 0, 0, width, height); URL.revokeObjectURL(url); resolve(canvas); }; image.onerror = () => { URL.revokeObjectURL(url); resolve(null); }; image.src = url; }); }
...
type StudioProps = { title:string; en:boolean; close:()=>void; color:string; setColor:(value:string)=>void; eraser:boolean; setEraser:(value:boolean)=>void; undo:number; redo:number; undoOne:()=>void; redoOne:()=>void; reset:()=>void; getCanvas:()=>Promise<HTMLCanvasElement|null>; children:React.ReactNode };
