...
export function VideoCreateManager({ children }: { children: React.ReactNode }) {
  ...
  return <>
    <header className="mx-auto flex max-w-6xl flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-xs font-black uppercase tracking-[.2em] text-emerald-700">Contenu vidéo</p><h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950 md:text-4xl">Vidéos</h1><p className="mt-2 max-w-2xl text-slate-500">Gérez les vidéos, leurs miniatures et leur publication depuis une seule liste.</p></div>
      <button type="button" onClick={() => setOpen(true)} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 font-black text-white shadow-lg transition hover:-translate-y-0.5 hover:bg-emerald-800 hover:shadow-xl"><Plus size={19}/>Créer une vidéo</button>
    </header>
    {children}
...
function CreateVideoDialog({ close }: { close: () => void }) {
  ...
  return <div className="fixed inset-0 z-[120] grid place-items-center bg-slate-950/75 p-3 backdrop-blur-md sm:p-5" onMouseDown={dismiss}>
    <form ref={formRef} onSubmit={submit} onMouseDown={event => event.stopPropagation()} className="max-h-[94vh] w-full max-w-6xl overflow-auto rounded-[2rem] bg-white shadow-[0_35px_120px_-30px_rgba(0,0,0,.8)]">
      <header className="flex items-start justify-between gap-5 border-b bg-gradient-to-br from-emerald-50 via-white to-white px-6 py-5 sm:px-8"><div className="flex gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-emerald-100 text-emerald-700"><Film size={22}/></span><div><p className="text-[11px] font-black uppercase tracking-[.2em] text-emerald-700">Nouvelle publication</p><h2 className="mt-1 text-2xl font-black tracking-tight text-slate-950">Créer une vidéo</h2><p className="mt-1 text-sm text-slate-500">Ajoutez le contenu et contrôlez son rendu avant publication.</p></div></div><button type="button" aria-label="Fermer" onClick={dismiss} className="grid size-10 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-500 transition hover:bg-slate-200"><X size={19}/></button></header>
      ...
      <div className="grid gap-6 px-6 py-6 sm:px-8 lg:grid-cols-[1fr_.95fr]">
        <div className="space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border bg-slate-50 p-4"><p className="flex items-center gap-2 text-sm font-black text-slate-800"><Clock3 size={16} className="text-emerald-700"/>Durée détectée</p><p className="mt-2 text-2xl font-black text-slate-950">{duration ? formatDuration(duration) : "—"}</p><p className="mt-1 text-xs text-slate-500">Calculée automatiquement depuis le MP4.</p></div><label className={`flex cursor-pointer items-center justify-between gap-3 rounded-2xl border p-4 transition ${published ? "border-emerald-200 bg-emerald-50" : "bg-slate-50"}`}><span><span className="block text-sm font-black text-slate-900">Publier sur le site</span><span className="mt-1 block text-xs text-slate-500">{published ? "Visible immédiatement" : "Conserver en brouillon"}</span></span><Switch checked={published}/><input className="sr-only" type="checkbox" checked={published} onChange={event => setPublished(event.target.checked)}/></label></div>
          <label className="flex cursor-pointer items-center gap-3 rounded-2xl border bg-slate-50 p-4"><input type="checkbox" checked={featured} onChange={event => setFeatured(event.target.checked)} className="size-5 accent-emerald-700"/><span><span className="block text-sm font-black text-slate-900">Mettre cette vidéo en avant</span><span className="mt-1 block text-xs text-slate-500">Elle apparaîtra prioritairement dans les listes.</span></span></label>
          {error && <p role="alert" className="rounded-2xl bg-red-50 p-4 text-sm font-bold text-red-700">{error}</p>}
        </div>

        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
          <FilePreview title="Vidéo MP4" hint="MP4 · 1 Go maximum" selected={videoFile?.name} icon={<Film size={28}/>}>
      ...
      <footer className="flex flex-col-reverse gap-3 border-t bg-slate-50/80 px-6 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-8"><p className="hidden items-center gap-2 text-xs font-semibold text-slate-500 sm:flex"><Sparkles size={15} className="text-emerald-700"/>Les fichiers sont vérifiés avant leur stockage.</p><div className="flex flex-col-reverse gap-3 sm:flex-row"><button type="button" onClick={dismiss} disabled={loading} className="min-h-12 rounded-2xl border bg-white px-6 font-bold text-slate-600">Annuler</button><button disabled={loading || !videoFile} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-6 font-black text-white shadow-lg transition hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-45">{loading ? <LoaderCircle className="animate-spin" size={18}/> : <CheckCircle2 size={18}/>} {loading ? "Création…" : "Créer la vidéo"}</button></div></footer>
    </form>
  </div>;
}
