...
export function SiteFooter() {
  ...
  return <footer className="relative mt-24 overflow-hidden border-t bg-card dark:border-white/10">
    ...
    <div className="container relative grid gap-10 py-14 lg:grid-cols-[1.05fr_1fr_.9fr] lg:gap-14 lg:py-16">
      <div>
        <Link href={localizedPath("home",locale)} className="focus-ring group inline-flex items-center gap-3 rounded-xl font-display text-2xl font-black dark:text-slate-50">
        ...
        <p className="mt-5 max-w-md text-base leading-7 text-foreground/60 dark:text-slate-300">{en?"Books and activities designed to nurture creativity, independence and focus, away from screens.":"Des livres et activités imaginés pour nourrir la créativité, l’autonomie et la concentration, loin des écrans."}</p>
        <Link href={localizedPath("about",locale)} className={cn("focus-ring group mt-6 inline-flex min-h-11 items-center gap-2 rounded-full border px-5 text-sm font-black transition duration-300 hover:-translate-y-0.5 hover:border-primary hover:bg-primary hover:text-white dark:border-white/15 dark:text-slate-100", pathname === localizedPath("about",locale) && "border-primary bg-primary text-white dark:text-white")}>
      ...
      <div className="rounded-[1.75rem] border bg-background/70 p-6 shadow-[0_18px_50px_-40px_rgba(15,23,42,.5)] backdrop-blur dark:border-white/10">
        ...
        <p className="mt-5 font-display text-lg font-black dark:text-slate-50">{en?"For parents":"Pour les parents"}</p>
        <p className="mt-2 text-sm leading-6 text-foreground/60 dark:text-slate-300">{en?"Gentle news, free resources and creative stories to enjoy as a family.":"Nouveautés douces, ressources gratuites et coulisses créatives à découvrir en famille."}</p>
    ...
    <div className="relative border-t bg-gradient-to-r from-amber-50/70 via-background to-emerald-50/70 dark:border-white/10 dark:from-amber-400/5 dark:via-background dark:to-emerald-400/5">
      <div className="container grid items-center gap-4 py-5 text-center text-sm md:grid-cols-[1fr_auto_1fr] md:text-left">
        <p className="inline-flex items-center justify-center gap-2 font-bold text-foreground/55 dark:text-slate-300 sm:justify-start">
        ...
        <nav aria-label={en ? "Useful links" : "Liens pratiques"} className="flex flex-wrap items-center justify-center gap-2">
          <Link href={localizedPath("about",locale)} className="focus-ring inline-flex min-h-9 items-center gap-2 rounded-full px-3 font-bold text-foreground/55 transition hover:bg-amber-100 hover:text-amber-800 dark:text-slate-300 dark:hover:bg-amber-400/15 dark:hover:text-amber-300"><Heart size={14} className="text-rose-500" fill="currentColor"/> {en?"About":"À propos"}</Link>
          <span className="size-1 rounded-full bg-foreground/20" aria-hidden="true"/>
          <a href={INSTAGRAM_URL} target="_blank" rel="noreferrer" className="focus-ring inline-flex min-h-9 items-center gap-2 rounded-full px-3 font-bold text-foreground/55 transition hover:bg-rose-100 hover:text-rose-700 dark:text-slate-300 dark:hover:bg-rose-400/15 dark:hover:text-rose-300"><Instagram size={15}/> Instagram</a>
        </nav>
