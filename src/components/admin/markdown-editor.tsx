...
export function MarkdownEditor({ value, onChange, name, label="Contenu Markdown", help="Rédigez, puis vérifiez le rendu avec l’aperçu.", maximumLength=100000, minimumLength=10, required=true, compact=false }: MarkdownEditorProps) {
  ...
  return <div>
    ...
    <div className="mt-2 flex items-center justify-between gap-3 text-xs font-semibold text-slate-400 dark:text-slate-500"><span>{minimumLength} caractères minimum</span><span className={`rounded-full px-2.5 py-1 tabular-nums ${remaining <= Math.min(1000, maximumLength*.1) ? "bg-amber-100 text-amber-800 dark:bg-amber-400/10 dark:text-amber-300" : "bg-slate-100 dark:bg-white/[.06] dark:text-slate-300"}`}>{remaining.toLocaleString("fr-FR")} restants</span></div>
