...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const parsed=schema.safeParse(await request.json());if(!parsed.success)return Response.json({message:parsed.error.issues[0]?.message},{status:400});
  ...
  await db.update(posts).set({...parsed.data,publishedAt:parsed.data.published?(current.publishedAt??new Date()):null,updatedAt:new Date()}).where(eq(posts.id,id));
