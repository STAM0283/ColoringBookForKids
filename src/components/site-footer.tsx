...
const links = [
  { label: "Les livres", href: "/livres", icon: BookOpen },
  { label: "Activités gratuites", href: "/activites", icon: Palette },
  { label: "Conseils créatifs", href: "/blog", icon: Lightbulb },
...
export function SiteFooter() {
  ...
  return <footer className="relative mt-24 overflow-hidden border-t bg-card dark:border-white/10">
    ...
    <div className="container relative grid gap-10 py-14 lg:grid-cols-[1.05fr_1fr_.9fr] lg:gap-14 lg:py-16">
      <div>
        <Link href="/" className="focus-ring group inline-flex items-center gap-3 rounded-xl font-display text-2xl font-black">
        ...
        <p className="mt-5 max-w-md text-base leading-7 text-foreground/60">Des livres et activités imaginés pour nourrir la créativité, l’autonomie et la concentration, loin des écrans.</p>
        <Link href="/a-propos" className={cn("focus-ring group mt-6 inline-flex min-h-11 items-center gap-2 rounded-full border px-5 text-sm font-black transition duration-300 hover:-translate-y-0.5 hover:border-primary hover:bg-primary hover:text-white", pathname === "/a-propos" && "border-primary bg-primary text-white")}>
          Notre histoire <ArrowUpRight className="transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" size={17}/>
        </Link>
      </div>

      <nav aria-label="Navigation du pied de page">
        <p className="font-display text-lg font-black">Explorer</p>
      ...
      <div className="rounded-[1.75rem] border bg-background/70 p-6 shadow-[0_18px_50px_-40px_rgba(15,23,42,.5)] backdrop-blur dark:border-white/10">
        ...
        <p className="mt-5 font-display text-lg font-black">Pour les parents</p>
        <p className="mt-2 text-sm leading-6 text-foreground/60">Nouveautés douces, ressources gratuites et coulisses créatives à découvrir en famille.</p>
        <Link href="/activites" className="focus-ring group mt-5 inline-flex items-center gap-2 rounded-lg text-sm font-black text-primary hover:underline hover:underline-offset-4">Trouver une activité <ArrowUpRight className="transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" size={16}/></Link>
      </div>
    </div>
    ...
    <div className="relative border-t bg-gradient-to-r from-amber-50/70 via-background to-emerald-50/70 dark:border-white/10 dark:from-amber-400/5 dark:via-background dark:to-emerald-400/5">
      <div className="container grid items-center gap-4 py-5 text-center text-sm md:grid-cols-[1fr_auto_1fr] md:text-left">
        ...
        <nav aria-label="Liens pratiques" className="flex flex-wrap items-center justify-center gap-2">
          <Link href="/a-propos" className="focus-ring inline-flex min-h-9 items-center gap-2 rounded-full px-3 font-bold text-foreground/55 transition hover:bg-amber-100 hover:text-amber-800 dark:hover:bg-amber-400/15 dark:hover:text-amber-300"><Heart size={14} className="text-rose-500" fill="currentColor"/> À propos</Link>
        ...
        <button type="button" onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} className="focus-ring group mx-auto inline-flex min-h-10 items-center gap-2 rounded-full border border-primary/15 bg-card px-4 font-black text-primary shadow-sm transition hover:-translate-y-0.5 hover:border-primary hover:bg-primary hover:text-white md:ml-auto md:mr-0">
          Retour en haut <span className="grid size-6 place-items-center rounded-full bg-primary/10 transition group-hover:bg-white/20"><ArrowUp size={14}/></span>
        </button>
      </div>
    </div>
  </footer>;
