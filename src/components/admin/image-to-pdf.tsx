...
import { ArrowDown, ArrowUp, CheckCircle2, FileImage, LoaderCircle, LockKeyhole, Trash2, UploadCloud, Users } from "lucide-react";
...
export function ImageToPdf() {
    ...
    async function convert() { setLoading(true); setError(""); setSuccess(""); const form = new FormData(); form.set("language", language); form.set("title", title); form.set("description", description); form.set("accessLevel", accessLevel); form.set("categoryIds", JSON.stringify(categoryIds)); if (cover)
    ...
    return <section className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm"><div className="border-b bg-gradient-to-r from-emerald-50 to-white p-6 md:p-8"><span className="grid size-12 place-items-center rounded-2xl bg-emerald-700 text-white"><FileImage /></span><h2 className="mt-5 text-2xl font-black">Ajouter une activité</h2><p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-500">Choisissez vos images. Elles seront converties en un PDF et ajoutées automatiquement aux activités.</p></div><div className="p-6 md:p-8"><label className="block text-sm font-bold text-slate-700">Langue<select value={language} onChange={event => setLanguage(event.target.value as "FR" | "EN")} className="mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-4"><option value="FR">🇫🇷 Français</option><option value="EN">🇬🇧 Anglais</option></select></label><label className="mt-5 block text-sm font-bold text-slate-700">Nom de l’activité<input value={title} onChange={event => setTitle(event.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-4 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100"/></label>
<div className="mt-5"><MarkdownEditor label="Description" value={description} onChange={setDescription} maximumLength={2000} compact help="Structurez la présentation avec du gras, des listes ou des sous-titres."/></div>
